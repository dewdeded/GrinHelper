...
txsecret=`cat send.html|sed 's/<br>Ok//g'`
      ...
      *) echo "Please answer yes or no." ;;
      ...
      if (( $amn > 10000 )) ; 
        then exec >&2; echo "error: Amount to big"; exit 1
      fi

      if (( $amn < 1 )) ; 
        then exec >&2; echo "error: Amount to small"; exit 1
      fi
...
tree > finaltx.txt
openssl enc -in finaltx.txt \
    -aes-256-cbc \
    -k send.html > finalsend.enc
...
while :; do
	...
	c) function_Check_Complettion ;;
	r) function_Retrieve_Secret ;;
