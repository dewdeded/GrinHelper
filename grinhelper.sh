...
sudo apt-get install build-essential curl cmake figlet jq -y	
...
option_s()
{
	...
	echo " "
	if ps aux | grep -q "[m]y_mining_server" ; then echo "Last graph time:		$(grep Graphs $PathGrinLogFile|tail -n 1 | awk ' { print $15 } ' | sed -e 's/;//g')"
	echo "Graphs per second:		$(grep Graphs $PathGrinLogFile|tail -n 1 | awk ' { print $19 } ' )" ; 
	else echo " "
	echo "Mining server is NOT running" ; fi
...
## Check if CLI argument passed to direct start node
if [ "$1" == "my_wallet" ];
then
    my_wallet
fi

## Check if CLI argument passed to direct start server
if [ "$1" == "my_mining_server" ];
then
	my_mining_server
	exit 1;
fi
...
## Check if CLI argument passed to view remote balance
if [ "$1" == "remote_balance" ];
then
	cd $HOME/mw/grin/node1
	export PATH=/$HOME/mw/grin/target/debug/:$PATH
